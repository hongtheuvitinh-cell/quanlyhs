...
import { 
  Search, X, User, Users, Sparkles, Edit2, Trash2, Save, Award, Phone, Calendar, CheckCircle2, AlertTriangle, Clock
} from 'lucide-react';
...
const StudentList: React.FC<Props> = ({ state, students, grades, onUpdateStudent, onDeleteStudent }) => {
  ...
  const getStudentAvg = (studentId: string) => {
    ...
    return (sGrades.reduce((sum, g) => sum + g.DiemSo, 0) / sGrades.length).toFixed(1);
  };

  return (
    <div className="space-y-4 animate-in fade-in pb-20">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white p-4 rounded-2xl border border-slate-200 shadow-sm">
        <h2 className="text-sm font-bold text-slate-800 flex items-center gap-2"><Users className="text-indigo-600" size={18} /> Danh sách lớp {state.selectedClass}</h2>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
        {filteredStudents.map((student) => {
          ...
          return (
            <div key={student.MaHS} className="bg-white p-4 rounded-3xl border border-slate-200 shadow-sm hover:border-indigo-200 transition-all group relative">
              <div className="flex items-start gap-4">
                <div className="h-14 w-14 rounded-2xl bg-indigo-50 flex items-center justify-center shrink-0 overflow-hidden border border-indigo-100">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-3 mt-2">
                     <div className="flex items-center gap-1 text-[10px] text-slate-400 font-bold"><Calendar size={12} /> {new Date(student.NgaySinh).toLocaleDateString('vi-VN')}</div>
                     <div className="flex items-center gap-1 text-[10px] text-slate-400 font-bold"><Phone size={12} /> {student.SDT_LinkHe}</div>
                  </div>
                </div>
              </div>
              ...
              <div className="mt-4 pt-4 border-t border-slate-50 flex items-center justify-between">
                ...
                <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                  <button onClick={() => onDeleteStudent(student.MaHS)} className="p-2 text-rose-500 hover:bg-rose-50 rounded-xl transition-all"><Trash2 size={16}/></button>
                </div>
              </div>
            </div>
          );
