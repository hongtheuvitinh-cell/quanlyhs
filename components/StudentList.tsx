    ...
    <div className="space-y-6 animate-in fade-in pb-20">
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 bg-white p-6 rounded-[32px] border border-slate-200 shadow-sm">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h2 className="text-sm font-black text-slate-800 uppercase tracking-tight">Danh sách học sinh lớp {state.selectedClass}</h2>
            <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest mt-1">Sĩ số: {sortedStudents.length} thành viên</p>
          </div>
        </div>
        <div className="flex flex-wrap items-center gap-2">
          ...
          <button onClick={() => { setFormData({}); setIsFormOpen(true); }} className="px-8 py-3 bg-indigo-600 text-white rounded-2xl shadow-xl shadow-indigo-100 hover:bg-indigo-700 active:scale-95 transition-all text-[10px] font-black uppercase tracking-widest">Thêm HS Mới</button>
        </div>
      </div>

      {/* Grid Card 3D - Phục hồi Aesthetic */}
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 2xl:grid-cols-4 gap-6">
        {sortedStudents.map((student, sIdx) => (
          <div key={student.MaHS} onClick={() => { setSelectedStudent(student); setActiveInfoTab('SYLL'); setAiResult(null); }} className="bg-white p-6 rounded-[40px] border border-slate-200 shadow-sm hover:shadow-2xl hover:border-indigo-400 hover:-translate-y-1 transition-all group cursor-pointer animate-in zoom-in duration-300">
            <div className="flex items-start gap-5">
              <div className="h-24 w-20 rounded-[28px] bg-slate-100 flex items-center justify-center shrink-0 border border-slate-200 overflow-hidden shadow-inner relative group-hover:ring-4 ring-indigo-50 transition-all">
              ...
              <div className="flex-1 min-w-0 py-1">
                <span className="text-[10px] font-black text-indigo-500 uppercase tracking-widest bg-indigo-50 px-2 py-0.5 rounded-lg mb-2 inline-block">{sIdx + 1}. {student.MaHS}</span>
                ...
                <div className="text-[11px] text-slate-400 font-bold flex flex-col gap-2">
                   <span className="flex items-center gap-2"><Calendar size={13} className="text-slate-300"/> {student.NgaySinh}</span>
                   <span className="flex items-center gap-2"><Phone size={13} className="text-slate-300"/> {student.SDT_LinkHe}</span>
                </div>
              </div>
            </div>
          </div>
      ...
      {/* Detail Modal - 4 Tab - Phục hồi SYLL & Password */}
      {selectedStudent && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-md animate-in fade-in">
          <div className="bg-white w-full max-w-5xl h-[90vh] rounded-[50px] shadow-2xl overflow-hidden flex flex-col animate-in zoom-in-95 border border-white/20">
            <div className="p-8 border-b flex items-center justify-between shrink-0 bg-white">
               <div className="flex items-center gap-6">
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-3">
                       <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest bg-slate-50 px-3 py-1 rounded-full border">ID: {selectedStudent.MaHS}</p>
               ...
               <div className="flex items-center gap-3">
                  <button onClick={() => { setFormData(selectedStudent); setIsFormOpen(true); }} className="p-3 bg-indigo-50 text-indigo-600 hover:bg-indigo-100 rounded-2xl transition-all"><Edit2 size={20}/></button>
                  <button onClick={() => onDeleteStudent(selectedStudent.MaHS)} className="p-3 bg-rose-50 text-rose-500 hover:bg-rose-100 rounded-2xl transition-all"><Trash2 size={20}/></button>
            ...
            <div className="flex-1 flex overflow-hidden">
               {/* Sidebar Modal */}
               <div className="w-72 border-r p-8 space-y-3 hidden md:block shrink-0 bg-slate-50/20">
                  {[
               ...
               <div className="flex-1 overflow-y-auto p-10 custom-scrollbar bg-white">
                  {aiResult && <div className="mb-10 p-6 bg-emerald-50 border border-emerald-100 rounded-[32px] text-[13px] text-emerald-800 font-medium italic leading-relaxed animate-in slide-in-from-top-4 shadow-sm relative"><Sparkles className="absolute -top-3 -right-3 text-emerald-400" size={24}/> AI: "{aiResult}"</div>}
                  ...
                  {activeInfoTab === 'SYLL' && (
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8 animate-in fade-in duration-500">
                       ...
                       <InfoField label="Mật khẩu học sinh" value={selectedStudent.MatKhau || '123456'} icon={<Lock size={16}/>} colorClass="text-indigo-600 bg-indigo-50/50" />
                       ...
                       <div className="md:col-span-2 grid grid-cols-2 gap-8 p-6 bg-slate-50/50 rounded-[40px] border border-slate-100">
                    ...
                    <div className="bg-white rounded-[40px] border border-slate-200 overflow-hidden shadow-sm animate-in fade-in">
                       <table className="w-full text-left">
                          <thead className="bg-slate-50 text-[10px] font-black text-slate-400 uppercase tracking-widest border-b">
                             <tr><th className="px-8 py-6">Môn học</th><th className="px-8 py-6 text-center">Học kỳ 1</th><th className="px-8 py-6 text-center">Học kỳ 2</th></tr>
                          </thead>
                          <tbody className="divide-y divide-slate-50">
                             {subjectsList.map(sub => (
                               <tr key={sub.id} className="hover:bg-slate-50 transition-colors">
                                  <td className="px-8 py-4.5 text-xs font-black text-slate-700 uppercase tracking-tight">{sub.name}</td>
                                  <td className="px-8 py-4.5 text-center font-black text-indigo-600 text-sm">{calculateSubjectAvg(selectedStudent.MaHS, sub.id, 1)?.toFixed(1) || '--'}</td>
                                  <td className="px-8 py-4.5 text-center font-black text-indigo-600 text-sm">{calculateSubjectAvg(selectedStudent.MaHS, sub.id, 2)?.toFixed(1) || '--'}</td>
                               </tr>
                             ))}
                          </tbody>
                       </table>
                  ...
                  {/* ... các tab Kỷ luật / Nhật ký tương tự ... */}
               </div>
            </div>
          </div>
        </div>
        ...
        <div className="fixed inset-0 z-[70] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-md animate-in fade-in">
           <div className="bg-white w-full max-w-2xl rounded-[50px] p-12 shadow-2xl overflow-y-auto max-h-[90vh] custom-scrollbar border border-white/20">
              <div className="flex items-center justify-between mb-10">
                 <h3 className="text-xl font-black text-slate-800 uppercase tracking-widest">Hồ sơ học sinh chi tiết</h3>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                 <InputField label="Họ và tên" value={formData.Hoten} onChange={(v:any) => setFormData({...formData, Hoten: v})} />
                 <InputField label="Mã HS (ID đăng nhập)" value={formData.MaHS} onChange={(v:any) => setFormData({...formData, MaHS: v})} disabled={!!formData.MaHS} />
