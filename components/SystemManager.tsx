...
const TaskManager: React.FC<Props> = ({ state, students, tasks, onUpdateTasks, onDeleteTask }) => {
  ...
  // Lọc danh sách nhiệm vụ dựa trên các tiêu chí
  const currentTasks = useMemo(() => {
    const today = new Date().toISOString().split('T')[0];
    
    return tasks.filter(t => {
      // 1. Lọc theo lớp hiện tại
      if (t.MaLop !== state.selectedClass) return false;
      
      // 2. Lọc theo niên học
      if (t.MaNienHoc !== state.selectedYear) return false;

      // 3. Lọc theo vai trò & môn học
      if (state.currentRole === Role.GIANG_DAY) {
        // Nếu là GV bộ môn, chỉ xem các bài của môn mình hoặc môn đang chọn
  ...
  }, [tasks, state.selectedClass, state.selectedYear, state.selectedSubject, state.currentRole, filterMonth, filterStatus, state.currentUser]);

  // Tự động cập nhật selectedTask khi tasks từ cha (App.tsx) thay đổi
  useEffect(() => {
    if (selectedTask) {
  ...
  const handleOpenAdd = () => {
    ...
    setTaskForm({ 
      ...
      MoTa: '', 
      HanChot: new Date().toISOString().split('T')[0], 
      MaMonHoc: state.selectedSubject || (state.currentUser as Teacher).MaMonChinh || 'SHL'
    });
  ...
  const handleSaveTask = async () => {
    ...
    try {
      ...
      // CRITICAL: Cập nhật vùng xem chi tiết ngay lập tức
  ...
  return (
    <div className="space-y-4 animate-in fade-in pb-20">
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 bg-white p-4 rounded-3xl border border-slate-200 shadow-sm">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest mt-1">Lớp {state.selectedClass} • Môn: {taskForm.MaMonHoc}</p>
          </div>
        </div>
      ...
      <div className="bg-white p-4 rounded-3xl border border-slate-200 shadow-sm flex flex-wrap items-end gap-5">
         ...
         <button onClick={() => { setFilterMonth('all'); setFilterStatus('all'); }} className="px-4 py-2 text-[9px] font-black text-rose-500 uppercase hover:bg-rose-50 rounded-xl transition-all">Reset</button>
      </div>
          ...
          {currentTasks.length > 0 ? currentTasks.map(task => {
            ...
            return (
             ...
             <div className="py-24 bg-white rounded-[40px] border-2 border-dashed border-slate-100 text-center opacity-40 flex flex-col items-center justify-center">
                ...
                <p className="text-[11px] font-black text-slate-400 uppercase tracking-widest leading-relaxed text-center">Không có nhiệm vụ nào</p>
             </div>
          )}
        </div>
            ...
            <div className="bg-white rounded-[40px] border border-slate-200 shadow-sm overflow-hidden flex flex-col h-full min-h-[600px] animate-in slide-in-from-right-4">
                ...
                <div className="flex items-center gap-4">
                   ...
                   <div>
                     ...
                     <div className="flex items-center gap-3 mt-1.5">
                        ...
                        <p className="text-[10px] text-slate-400 font-black uppercase tracking-widest">Hạn: {selectedTask.HanChot}</p>
                     </div>
                   </div>
                </div>
              ...
              <div className="p-8 border-b bg-white">
                <div className="flex items-center gap-2 mb-3">
                   ...
                   <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Yêu cầu & Nội dung</p>
                </div>
                <div className="p-6 bg-slate-50 rounded-[32px] border border-slate-100 text-xs font-medium text-slate-700 leading-relaxed italic shadow-inner whitespace-pre-line">
                  "{selectedTask.MoTa || 'Không có mô tả chi tiết.'}"
                </div>
              </div>

              <div className="flex-1 p-8 overflow-y-auto custom-scrollbar bg-slate-50/20">
                <div className="flex items-center justify-between mb-6 px-2">
                   <p className="text-[11px] font-black text-slate-800 uppercase tracking-widest">Tiến độ nộp bài ({selectedTask.DanhSachHoanThanh.length}/{selectedTask.DanhSachGiao?.length})</p>
                </div>
                    ...
                    return (
                      <div key={s.MaHS} className={`p-4 rounded-3xl border transition-all flex items-center justify-between group shadow-sm ${isDone ? 'bg-emerald-50/50 border-emerald-200' : 'bg-white border-slate-100'}`}>
                        <div className="flex items-center gap-4">
                           ...
                           <div>
                             <span className="text-xs font-black text-slate-700 uppercase tracking-tight">{s.Hoten}</span>
                             {link && <a href={link} target="_blank" rel="noopener noreferrer" className="block text-[9px] text-indigo-500 hover:underline mt-1 font-black uppercase tracking-widest">Link bài làm &rarr;</a>}
                           </div>
                        </div>
          ...
          <div className="bg-white w-full max-w-3xl rounded-[40px] shadow-2xl overflow-hidden animate-in zoom-in-95 flex flex-col max-h-[90vh]">
            <div className="p-6 border-b flex items-center justify-between bg-white shrink-0">
               <div className="flex items-center gap-3">
                  ...
                  <h3 className="font-black text-base text-slate-800 uppercase tracking-tight">{modalMode === 'add' ? 'Phát hành nhiệm vụ mới' : 'Cập nhật nhiệm vụ'}</h3>
               </div>
            ...
            <div className="flex-1 overflow-y-auto p-10 custom-scrollbar space-y-8 bg-slate-50/20">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                 <div className="space-y-6">
                    <h5 className="text-[11px] font-black text-slate-400 uppercase tracking-widest px-1">Thông tin chi tiết</h5>
                    <div className="space-y-5">
                       <div className="space-y-1.5">
                          <label className="text-[10px] font-black text-slate-500 uppercase px-1 tracking-widest">Tiêu đề</label>
                          <input type="text" value={taskForm.TieuDe} onChange={e => setTaskForm({...taskForm, TieuDe: e.target.value})} className="w-full p-4 bg-white border border-slate-200 rounded-2xl text-xs font-bold outline-none focus:border-indigo-400 shadow-sm transition-all" />
                       </div>
                       <div className="grid grid-cols-2 gap-5">
                          <div className="space-y-1.5">
                            <label className="text-[10px] font-black text-slate-500 uppercase px-1 tracking-widest">Môn học</label>
                            <select value={taskForm.MaMonHoc} onChange={e => setTaskForm({...taskForm, MaMonHoc: e.target.value})} className="w-full p-4 bg-white border border-slate-200 rounded-2xl text-xs font-black outline-none text-indigo-600">
                              {subjects.map(s => <option key={s.id} value={s.id}>{s.name}</option>)}
                            </select>
                          </div>
                          <div className="space-y-1.5">
                            <label className="text-[10px] font-black text-slate-500 uppercase px-1 tracking-widest">Hạn chót</label>
                            <input type="date" value={taskForm.HanChot} onChange={e => setTaskForm({...taskForm, HanChot: e.target.value})} className="w-full p-4 bg-white border border-slate-200 rounded-2xl text-xs font-black outline-none" />
                          </div>
                       </div>
                       <div className="space-y-1.5">
                          <label className="text-[10px] font-black text-slate-500 uppercase px-1 tracking-widest">Nội dung yêu cầu</label>
                          <textarea value={taskForm.MoTa} onChange={e => setTaskForm({...taskForm, MoTa: e.target.value})} className="w-full p-5 bg-white border border-slate-200 rounded-[32px] h-40 text-xs font-medium outline-none focus:border-indigo-400 transition-all" placeholder="Enter để xuống dòng..."></textarea>
                       </div>
                    </div>
                 </div>
                 ...
                 <div className="space-y-6">
                    <div className="flex items-center justify-between px-1">
                       <h5 className="text-[11px] font-black text-slate-400 uppercase tracking-widest">Giao cho ai?</h5>
                       <div className="flex gap-4">
                          <button onClick={() => setAssignedStudentIds(students.map(s => s.MaHS))} className="text-[10px] font-black text-indigo-600 uppercase">Tất cả</button>
                          <button onClick={() => setAssignedStudentIds([])} className="text-[10px] font-black text-rose-500 uppercase">Hủy</button>
                       </div>
                    </div>
                    <div className="space-y-4">
                       <input type="text" placeholder="Tìm tên nhanh..." value={studentFilter} onChange={e => setStudentFilter(e.target.value)} className="w-full px-5 py-3.5 bg-white border border-slate-200 rounded-2xl text-[12px] font-bold outline-none" />
                       <div className="bg-white border border-slate-200 rounded-[32px] max-h-[350px] overflow-y-auto p-3 custom-scrollbar">
                          {filteredStudentsForModal.map(s => (
                            <button key={s.MaHS} onClick={() => toggleStudent(s.MaHS)} className={`w-full flex items-center justify-between p-3.5 rounded-2xl mb-2 transition-all ${assignedStudentIds.includes(s.MaHS) ? 'bg-indigo-50 border-indigo-100' : 'hover:bg-slate-50 border-transparent'}`}>
                               <div className="flex items-center gap-4">
                                  <div className={`w-9 h-9 rounded-xl flex items-center justify-center text-[11px] font-black ${assignedStudentIds.includes(s.MaHS) ? 'bg-indigo-600 text-white' : 'bg-slate-100 text-slate-400'}`}>{s.Hoten.charAt(0)}</div>
                                  <span className={`text-[11px] font-black uppercase ${assignedStudentIds.includes(s.MaHS) ? 'text-indigo-800' : 'text-slate-500'}`}>{s.Hoten}</span>
                               </div>
                               {assignedStudentIds.includes(s.MaHS) ? <CheckCircle size={18} className="text-indigo-600" /> : <Circle size={18} className="text-slate-200" />}
                            </button>
                          ))}
                       </div>
                    </div>
            ...
            <div className="p-8 bg-white border-t flex justify-end gap-4 shrink-0">
               <button onClick={() => setIsModalOpen(false)} className="px-8 py-4 bg-slate-50 text-slate-500 rounded-2xl font-black text-[11px] uppercase tracking-widest hover:bg-slate-100 transition-all">Hủy</button>
               <button onClick={handleSaveTask} disabled={isSubmitting} className="px-12 py-4 bg-indigo-600 text-white rounded-2xl font-black shadow-xl shadow-indigo-100 hover:bg-indigo-700 active:scale-95 transition-all flex items-center gap-3 text-[11px] uppercase tracking-widest">
                  ...
                  {modalMode === 'add' ? 'Phát hành' : 'Lưu cập nhật'}
               </button>
            </div>
          </div>
        </div>
