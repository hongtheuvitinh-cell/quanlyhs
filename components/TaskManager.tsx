  ...
  return (
    <div className="space-y-4 animate-in fade-in pb-20">
      ...
      <div className="bg-white p-4 rounded-3xl border border-slate-200 shadow-sm flex flex-wrap items-end gap-5">
         ...
         <div className="space-y-1.5 flex-1 min-w-[150px]">
            <label className="text-[9px] font-black text-slate-400 uppercase px-1 tracking-widest">Trạng thái hạn</label>
         ...
         <button onClick={() => { setFilterMonth('all'); setFilterStatus('all'); }} className="px-4 py-2 text-[9px] font-black text-rose-500 uppercase hover:bg-rose-50 rounded-xl transition-all">Xóa lọc</button>
      </div>
              ...
              <div key={task.MaNhiemVu} onClick={() => setSelectedTask(task)} className={`p-6 rounded-[32px] border transition-all cursor-pointer group relative overflow-hidden ${isSelected ? 'bg-indigo-600 border-indigo-600 text-white shadow-2xl shadow-indigo-100 scale-[1.02]' : 'bg-white border-slate-200 hover:border-indigo-300 shadow-sm'}`}>
                ...
                <div className="space-y-2">
                  <div className="flex justify-between items-center text-[9px] font-black uppercase tracking-widest">
                    <span className={isSelected ? 'text-indigo-100' : 'text-slate-400'}>Nộp: {task.DanhSachHoanThanh.length}/{task.DanhSachGiao?.length}</span>
                    <span>{progress}%</span>
                  </div>
            ...
            <div className="bg-white rounded-[40px] border border-slate-200 shadow-sm overflow-hidden flex flex-col h-full min-h-[600px] animate-in slide-in-from-right-4">
              ...
              <div className="p-8 border-b bg-white">
                <div className="flex items-center gap-2 mb-3">
                   ...
                   <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Nội dung chi tiết</p>
                </div>
              ...
              <div className="flex-1 p-8 overflow-y-auto custom-scrollbar bg-slate-50/20">
                <div className="flex items-center justify-between mb-6 px-2">
                   <p className="text-[11px] font-black text-slate-800 uppercase tracking-widest">Kết quả nộp bài ({selectedTask.DanhSachHoanThanh.length}/{selectedTask.DanhSachGiao?.length})</p>
                </div>
                      ...
                      <div key={s.MaHS} className={`p-4 rounded-3xl border transition-all flex items-center justify-between group shadow-sm ${isDone ? 'bg-emerald-50/50 border-emerald-200' : 'bg-white border-slate-100'}`}>
                        <div className="flex items-center gap-4">
                           ...
                           <div className="min-w-0">
                             <span className="text-xs font-black text-slate-700 uppercase tracking-tight block truncate">{s.Hoten}</span>
                             {link && <a href={link} target="_blank" rel="noopener noreferrer" className="block text-[9px] text-indigo-500 hover:underline mt-1 font-black uppercase tracking-widest truncate max-w-[120px]">Xem bài nộp &rarr;</a>}
                           </div>
                        </div>
                        <div className={`flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-[9px] font-black uppercase tracking-widest shadow-sm shrink-0 ${isDone ? 'bg-emerald-600 text-white' : 'bg-slate-100 text-slate-400 opacity-60'}`}>
          ...
          <div className="bg-white w-full max-w-3xl rounded-[40px] shadow-2xl overflow-hidden animate-in zoom-in-95 flex flex-col max-h-[90vh]">
            <div className="p-6 border-b flex items-center justify-between bg-white shrink-0">
               <div className="flex items-center gap-3">
                  ...
                  <h3 className="font-black text-base text-slate-800 uppercase tracking-tight">{modalMode === 'add' ? 'Tạo nhiệm vụ học tập' : 'Chỉnh sửa nhiệm vụ'}</h3>
               </div>
            ...
            <div className="flex-1 overflow-y-auto p-10 custom-scrollbar space-y-8 bg-slate-50/20">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                 <div className="space-y-6">
                    <h5 className="text-[11px] font-black text-slate-400 uppercase tracking-widest px-1">Cấu hình bài tập</h5>
                    <div className="space-y-5">
                       <div className="space-y-1.5">
                          <label className="text-[10px] font-black text-slate-500 uppercase px-1 tracking-widest">Tên nhiệm vụ</label>
                       ...
                       <div className="grid grid-cols-2 gap-5">
                          ...
                          <div className="space-y-1.5">
                            <label className="text-[10px] font-black text-slate-500 uppercase px-1 tracking-widest">Ngày hết hạn</label>
                       ...
                       <div className="space-y-1.5">
                          <label className="text-[10px] font-black text-slate-500 uppercase px-1 tracking-widest">Mô tả chi tiết</label>
                          <textarea value={taskForm.MoTa} onChange={e => setTaskForm({...taskForm, MoTa: e.target.value})} className="w-full p-5 bg-white border border-slate-200 rounded-[32px] h-40 text-xs font-medium outline-none focus:border-indigo-400 transition-all" placeholder="Yêu cầu cụ thể của bài tập..."></textarea>
                       </div>
                    </div>
                 </div>
                 ...
                 <div className="space-y-6">
                    <div className="flex items-center justify-between px-1">
                       <h5 className="text-[11px] font-black text-slate-400 uppercase tracking-widest">Đối tượng nhận bài</h5>
                       <div className="flex gap-4">
                          <button onClick={() => setAssignedStudentIds(students.map(s => s.MaHS))} className="text-[10px] font-black text-indigo-600 uppercase hover:underline">Tất cả</button>
                          <button onClick={() => setAssignedStudentIds([])} className="text-[10px] font-black text-rose-500 uppercase hover:underline">Hủy chọn</button>
                       </div>
                    </div>
                    <div className="space-y-4">
                       <div className="relative">
                          <Search className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-300" size={16} />
                          <input type="text" placeholder="Tìm tên học sinh..." value={studentFilter} onChange={e => setStudentFilter(e.target.value)} className="w-full pl-10 pr-4 py-3 bg-white border border-slate-200 rounded-2xl text-[12px] font-bold outline-none shadow-sm" />
                       </div>
            ...
            <div className="p-8 bg-white border-t flex justify-end gap-4 shrink-0">
               <button onClick={() => setIsModalOpen(false)} className="px-10 py-4 bg-slate-50 text-slate-500 rounded-2xl font-black text-[11px] uppercase tracking-widest hover:bg-slate-200 transition-all">Đóng</button>
               <button onClick={handleSaveTask} disabled={isSubmitting} className="px-14 py-4 bg-indigo-600 text-white rounded-2xl font-black shadow-xl shadow-indigo-100 hover:bg-indigo-700 active:scale-95 transition-all flex items-center gap-3 text-[11px] uppercase tracking-widest">
                  {isSubmitting ? <Loader2 size={20} className="animate-spin" /> : <Save size={20}/>}
                  {modalMode === 'add' ? 'Phát hành bài tập' : 'Lưu cập nhật'}
               </button>
            </div>
          </div>
        </div>
