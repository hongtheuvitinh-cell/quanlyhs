...
const App: React.FC = () => {
  ...
  // Lấy danh sách học sinh dựa trên lớp đã chọn - Đảm bảo cuộn vô cực, không giới hạn dòng
  const currentClassStudents = useMemo(() => {
    if (!state.selectedClass || !students) return [];
    return students.filter(s => s.MaLopHienTai === state.selectedClass);
  }, [students, state.selectedClass]);

  // Lọc các lớp mà GV có quyền truy cập (Dựa trên Phân công + Vai trò hiện tại)
  const filteredClasses = useMemo(() => {
    if (!state.currentUser || (state.currentUser as any).MaHS) return [];
    const teacherID = (state.currentUser as Teacher).MaGV;
    
    // Tìm các phân công của GV này trong niên học đang chọn
    const myAs = assignments.filter(a => a.MaGV === teacherID && a.MaNienHoc === state.selectedYear);
    
    // Lọc các ID lớp dựa trên vai trò đang chọn (Chủ nhiệm hay Giảng dạy)
  ...
  // Cập nhật lớp mặc định khi đổi vai trò hoặc niên học
  useEffect(() => {
    if (filteredClasses.length > 0 && !filteredClasses.some(c => c.MaLop === state.selectedClass)) {
      setState(p => ({ ...p, selectedClass: filteredClasses[0].MaLop }));
    }
  }, [filteredClasses, state.currentRole, state.selectedYear]);
  ...
  const handleLogin = (role: Role, id: string, pass: string) => {
    if (role === Role.STUDENT) {
      ...
      if (t && (t.MatKhau || '123456') === pass) {
        ...
        if (myAs.length === 0) {
          alert("Giáo viên chưa có phân công trong hệ thống!");
          return;
        }
        // Ưu tiên vai trò chủ nhiệm nếu có
    ...
    <div className="flex h-screen bg-[#F8FAFC] overflow-hidden text-[13px] font-normal text-slate-600">
      ...
      <aside className="w-64 bg-white border-r border-slate-200 flex flex-col shrink-0 shadow-sm relative z-20">
        <div className="p-6 flex items-center gap-3 border-b border-slate-50">
          <div className="bg-indigo-600 p-2.5 rounded-2xl text-white shadow-lg shadow-indigo-100"><GraduationCap size={20} /></div>
        ...
        <nav className="flex-1 px-4 space-y-1.5 pt-6 overflow-y-auto custom-scrollbar">
          <div className="mb-4 px-4 py-2 bg-slate-50 rounded-2xl border border-slate-100">
             <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1.5">Vai trò làm việc</p>
          ...
          <button onClick={() => setActiveTab('system')} className={`w-full flex items-center gap-3 px-4 py-3 rounded-2xl font-black uppercase text-[10px] tracking-widest mt-2 transition-all ${activeTab === 'system' ? 'bg-slate-900 text-white shadow-xl' : 'text-slate-400 hover:bg-slate-50'}`}>
        ...
        <header className="h-16 bg-white border-b border-slate-200 px-8 flex items-center justify-between shrink-0 relative z-10">
          <div className="flex items-center gap-8">
            <div className="flex items-center gap-3">
              <span className="text-[10px] font-black text-slate-300 uppercase tracking-widest">Đang xem:</span>
              <div className="flex items-center gap-2">
                <select 
                  value={state.selectedClass} 
                  onChange={(e) => setState(p => ({...p, selectedClass: e.target.value}))} 
                  className="font-black text-slate-800 border-none outline-none bg-slate-50 px-3 py-1.5 rounded-xl text-xs shadow-inner cursor-pointer"
                >
                  {filteredClasses.length > 0 ? filteredClasses.map(c => <option key={c.MaLop} value={c.MaLop}>{c.TenLop}</option>) : <option>Trống</option>}
                </select>
                <div className="w-px h-4 bg-slate-200 mx-2"></div>
                <select 
                  value={state.selectedYear} 
                  onChange={(e) => setState(p => ({...p, selectedYear: parseInt(e.target.value)}))} 
                  className="font-black text-indigo-600 border-none outline-none bg-indigo-50 px-3 py-1.5 rounded-xl text-xs shadow-inner cursor-pointer"
                >
          ...
          <div className="flex items-center gap-4">
             <div className="text-right hidden sm:block">
                <p className="text-[11px] font-black text-slate-800 uppercase tracking-tight">{(state.currentUser as Teacher)?.Hoten}</p>
             ...
             <div className="w-10 h-10 rounded-2xl bg-indigo-600 flex items-center justify-center text-white font-black shadow-lg shadow-indigo-100 ring-4 ring-indigo-50">
               {(state.currentUser as Teacher)?.Hoten?.charAt(0)}
             </div>
          </div>
        </header>

        {/* Content Area - Đảm bảo cuộn mượt, không cắt dòng */}
